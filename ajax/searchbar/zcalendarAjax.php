...
function getPropertyAvailAndPrice() {
    ...
    $children_ages = $_GET['children_ages'];
    ...
    $adults_for_send = 0;
    $children_for_send = 0;
    $children_ages_for_send = null;

    for ( $i=0 ; $i < count($adults) ; $i++ ) {

        if ( ($adults[$i] + $children[$i]) < $referent_number ) {

            $referent_number = $adults[$i] + $children[$i];
            $adults_for_send = $adults[$i];
            $children_for_send = $children[$i];
            $children_ages_for_send = $children_ages[$i];
    ...
    $hotelAvailCalendar = BeApi::getHotelAvailCalendarV4($property, $date_from, $date_to, $currency ,$adults_for_send, $children_for_send, $children_ages_for_send);
    ...
    if ( isset($hotelAvailCalendar->HotelStaysType->HotelStays ) && $hotelAvailCalendar->HotelStaysType->HotelStays!=null ) {
        ...
        foreach( $hotelAvailCalendar->HotelStaysType->HotelStays as $stay ){
            ...
            if (isset($stay->Availability)) {
                $restrictions = $stay->Availability->Restrictions;
            } else {
                $restrictions = null;
            }
